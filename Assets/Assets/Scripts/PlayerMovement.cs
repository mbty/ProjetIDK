...
public class PlayerMovement : MonoBehaviour {
	...
	private const float SCROLL_SENSITIVITY = 8.0f;
	private const float ROTATION_SENSITIVITY = 3.0f;
	...
	void Update () {
		...
		if (true) {
			Vector3 mvt = new Vector3 (input_x * FORWARD_SPEED * Time.deltaTime * transform.forward, 0, input_z * SIDEWARD_SPEED * Time.deltaTime * transform.right);
	...
	void updateCamera () {
		...
		Vector3 rotateValue = new Vector3(input_y * ROTATION_SENSITIVITY, 0, 0);
		cm.transform.eulerAngles = cm.transform.eulerAngles - rotateValue;

		rb.transform.eulerAngles = new Vector3(0, Time.time * 15, 0); 

		Camera.main.fieldOfView += Input.GetAxis("Mouse ScrollWheel") * -SCROLL_SENSITIVITY;
		if (Camera.main.fieldOfView < MIN_FOV)
			Camera.main.fieldOfView = MIN_FOV;
		else if (Camera.main.fieldOfView > MAX_FOV)
